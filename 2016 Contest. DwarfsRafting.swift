...
struct Raft {
    ...
    var frontFreeSpace: Int { return quarterSize * 2 - (frontLeft.barrels + frontLeft.dwarfs + frontRight.barrels + frontRight.dwarfs) }    
    var backFreeSpace: Int { return quarterSize * 2 - (backLeft.barrels + backLeft.dwarfs + backRight.barrels + backRight.dwarfs) }
    var leftFreeSpace: Int { return quarterSize * 2 - (frontLeft.barrels + frontLeft.dwarfs + backLeft.barrels + backLeft.dwarfs) }
    var rightFreeSpace: Int { return quarterSize * 2 - (frontRight.barrels + frontRight.dwarfs + backRight.barrels + backRight.dwarfs) }
    var frontWeight: Int { return frontLeft.dwarfs + frontRight.dwarfs }
    var backWeight: Int { return backLeft.dwarfs + backRight.dwarfs }
    var leftWeight: Int { return frontLeft.dwarfs + backLeft.dwarfs }
    var rightWeight: Int { return frontRight.dwarfs + backRight.dwarfs }
    let size: Int
    let quarterSize: Int
    ...
    init(size: Int, barrels: String, occupiedSeats: String) {
        ...
        for barrel in barrels.components(separatedBy: " ") where barrels.count > 0 { placeBarrel(position: barrel) }
        for occupiedSeat in occupiedSeats.components(separatedBy: " ") where occupiedSeats.count > 0 { occupySeat(position: occupiedSeat) }
    }
    ...
    func isBalanced() -> Bool { return (frontWeight == backWeight) == (leftWeight == rightWeight) }
